...
class World(Scene):
    ...
    def __init__(self, viewer, **kwargs):
        ...
        self._home_position = kwargs.get(
            "home_position", QVector3D(0.0, 0.0, 1.0))

        # define a camera
        self._camera = Camera(
            name="main", position=self._home_position, lens=Camera.Lens.Perspective)
        ...
        camera = Camera(name="main", position=QVector3D(
            0.0, 0.0, 4.0), lens=Camera.Lens.Perspective)
        ...
        super(World, self).__init__(
            viewer, light=light, camera=camera, **kwargs)
    ...
    def initialize(self):
        ...
        self._backgroundActor = Background(
            self, name="background", palette=self._background)
    ...
    def createGridLines(self, properties=None):
        ...
        self._axisActor = Axis(
            self, name="axis", length_row=self._gridProperties['length_rows'], length_col=self._gridProperties['length_cols'])
    ...
    def recallCamera(self):
        ...
        self.camera.recall(self.viewer.width(
        ) / float(self.viewer.height() if self.viewer.height() > 0.0 else 1.0))

    def resetCamera(self):
        """Reset camera parameters"""
        self.camera.setPosition(self._home_position)
        self.camera.pointAt(QVector3D(0.0, 0.0, 0.0))
        self.camera.setAspectRatio(self.viewer.width(
        ) / float(self.viewer.height() if self.viewer.height() > 0.0 else 1.0))
