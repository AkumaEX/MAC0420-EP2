...
class  Trackball(QObject):
    ...
    def move(self, point, quat):
        ...
        if self._mode == self.TrackballMode.Planar:
            ...
            self._rotation = QQuaternion.fromAxisAndAngle(
                self._axis, 180.0 / math.pi * delta.length()) * self._rotation
            ...
            angle = 180.0 / math.pi * \
                math.asin(
                    math.sqrt(QVector3D.dotProduct(self._axis, self._axis)))

            self._angularVelocity = angle / msecs
            self._axis.normalize()
            self._axis = quat.rotatedVector(self._axis)
            self._rotation = QQuaternion.fromAxisAndAngle(
                self._axis, angle) * self._rotation
