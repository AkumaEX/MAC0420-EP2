...
class Icosahedron(Actor):
    ...
    def __init__(self, renderer,  **kwargs):
        ...
        if self._rgb_colors:
            self.setSolidShader(
                self.shaderCollection.attributeColorPhongShader())
            self.setSolidFlatShader(
                self.shaderCollection.attributeColorPhongFlatShader())
            self.setNoLightSolidShader(
                self.shaderCollection.attributeColorShader())
            self.setWireframeShader(
                self.shaderCollection.uniformMaterialShader())
        else:
            self.setSolidShader(
                self.shaderCollection.uniformMaterialPhongShader())
            self.setSolidFlatShader(
                self.shaderCollection.uniformMaterialPhongFlatShader())
            self.setNoLightSolidShader(
                self.shaderCollection.uniformMaterialShader())
            self.setWireframeShader(
                self.shaderCollection.uniformMaterialPhongShader())
    ...
    def getMiddlePoint(self, p1, p2, vertices, indexCache):
        ...
        point1 = vertices[p1]
        point2 = vertices[p2]
        middle = ((point1[0] + point2[0]) / 2.0, (point1[1] +
                                                  point2[1]) / 2.0, (point1[2] + point2[2]) / 2.0)
    ...
    def generateGeometry(self):
        ...
        for i in range(self._level):
            ...
            for tri in indices:
                ...
                a = self.getMiddlePoint(
                    tri[0], tri[1], vertices, middlePointIndexCache)
                b = self.getMiddlePoint(
                    tri[1], tri[2], vertices, middlePointIndexCache)
                c = self.getMiddlePoint(
                    tri[2], tri[0], vertices, middlePointIndexCache)
    ...
    def render(self):
        """Render icosahedron"""
        GL.glDrawElements(self._render_mode,
                          self.numberOfIndices, GL.GL_UNSIGNED_INT, None)
