...
class Cone(Actor):
    ...
    def generateGeometry(self):
        ...
        cosn = (self._height / np.sqrt(self._height *
                                       self._height + self._radius * self._radius))
        sinn = (self._radius / np.sqrt(self._height *
                                       self._height + self._radius * self._radius))
        ...
        for i in list(range(self._resolution)):
            ...
            normals.append(
                [(nx[i]+nx[i+1])*0.5, (ny[i]+ny[i+1])*0.5, (nz[i]+nz[i+1])*0.5])
